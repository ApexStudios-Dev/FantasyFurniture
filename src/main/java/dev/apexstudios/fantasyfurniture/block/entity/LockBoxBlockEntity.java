...
public final class LockBoxBlockEntity extends BaseBlockEntityComponentHolder {
    ...
    public LockBoxBlockEntity(BlockPos pos, BlockState blockState) {
        super(FurnitureBlockEntities.LOCKBOX.value(), pos, blockState);
    }

    @Override
    protected AbstractContainerMenu createMenu(int windowId, Player player) {
        return new SimpleMenu(FurnitureMenus.LOCKBOX.value(), windowId, player.getInventory(), getComponentOrThrow(BlockEntityComponentTypes.INVENTORY).getItemHandler(), ROWS);
    }
