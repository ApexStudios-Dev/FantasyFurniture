...
public final class ChestBlockEntity extends BaseBlockEntityComponentHolder {
    public static final int ROWS = 3;

    public ChestBlockEntity(BlockPos pos, BlockState blockState) {
        super(FurnitureBlockEntities.CHEST.value(), pos, blockState);
    }

    @Override
    protected AbstractContainerMenu createMenu(int windowId, Player player) {
        return new SimpleMenu(FurnitureMenus.CHEST.value(), windowId, player.getInventory(), getComponentOrThrow(BlockEntityComponentTypes.INVENTORY).getItemHandler(), ROWS);
    }
