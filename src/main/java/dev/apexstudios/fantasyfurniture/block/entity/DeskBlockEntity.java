...
public final class DeskBlockEntity extends BaseBlockEntityComponentHolder {
    ...
    public DeskBlockEntity(BlockPos pos, BlockState blockState) {
        super(FurnitureBlockEntities.DESK.value(), pos, blockState);
    }

    @Override
    protected AbstractContainerMenu createMenu(int windowId, Player player) {
        return new SimpleMenu(FurnitureMenus.DESK.value(), windowId, player.getInventory(), getComponentOrThrow(BlockEntityComponentTypes.INVENTORY).getItemHandler(), ROWS);
    }
