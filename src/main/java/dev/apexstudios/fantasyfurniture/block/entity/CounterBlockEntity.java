...
public final class CounterBlockEntity extends BaseBlockEntityComponentHolder {
    ...
    public CounterBlockEntity(BlockPos pos, BlockState blockState) {
        super(FurnitureBlockEntities.COUNTER.value(), pos, blockState);
    }

    @Override
    protected AbstractContainerMenu createMenu(int windowId, Player player) {
        return new SimpleMenu(FurnitureMenus.COUNTER.value(), windowId, player.getInventory(), getComponentOrThrow(BlockEntityComponentTypes.INVENTORY).getItemHandler(), ROWS);
    }
