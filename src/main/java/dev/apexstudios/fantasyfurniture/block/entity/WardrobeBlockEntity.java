...
public final class WardrobeBlockEntity extends BaseBlockEntityComponentHolder {
    ...
    public WardrobeBlockEntity(BlockPos pos, BlockState blockState) {
        super(FurnitureBlockEntities.WARDROBE.value(), pos, blockState);
    }

    @Override
    protected AbstractContainerMenu createMenu(int windowId, Player player) {
        return new SimpleMenu(FurnitureMenus.WARDROBE.value(), windowId, player.getInventory(), getComponentOrThrow(BlockEntityComponentTypes.INVENTORY).getItemHandler(), ROWS);
    }
