...
public interface CtmPack {
    ...
    final class Builder {
        ...
        private final Multimap<ProviderType<?>, Supplier<? extends BiConsumer<?, BaseRegistree<?>>>> providers = MultimapBuilder.hashKeys().linkedHashSetValues().build();
        ...
        public <TProvider> Builder providing(ProviderType<TProvider> providerType, Supplier<BiConsumer<TProvider, BaseRegistree<?>>> consumer) {
            providers.put(providerType, consumer);
            return this;
        }

        public <TProvider> Builder providing(ProviderType<TProvider> providerType, BiConsumer<TProvider, BaseRegistree<?>> consumer) {
        ...
        public CtmPack build() {
            return new CtmPack() {
                ...
                private final Multimap<ProviderType<?>, Supplier<? extends BiConsumer<?, BaseRegistree<?>>>> providers = HashMultimap.create(Builder.this.providers);
                ...
                private <TProvider> void provide(FeaturePackGenerator generator, ProviderType<TProvider> providerType) {
                    ...
                    generator.providing(providerType, (context, provider) -> CtmPacks.REFERENCES
                            .forEach(registree -> providers
                                    .get(providerType)
                                    .forEach(listener -> ((BiConsumer<TProvider, BaseRegistree<?>>) listener.get()).accept(provider, registree))
                            )
                    );
                }
            };
